...
def unwrap_4d(phase_w):
    ...
    lap_phase = np.cos(phase_w) * lap4(np.sin(phase_w), 1, mod) - \
        np.sin(phase_w) * lap4(np.cos(phase_w), 1, mod)
...
class MRI_4DFlow:
    ...
    def solve_for_velocity(self):
        ...
        # Solve for velocity
        self.velocity_estimate = np.matmul(
            self.DecodingMatrix*self.Venc, phase)
    ...
    def background_phase_correct(self, mag_thresh=0.08, angiogram_thresh=0.3, fit_order=2):
        ...
        pz, py, px = np.meshgrid(
            range(fit_order+1), range(fit_order+1), range(fit_order+1))
        ...
        z, y, x = np.meshgrid(np.linspace(-1, 1, self.velocity_estimate.shape[1]),
                              np.linspace(-1, 1,
                                          self.velocity_estimate.shape[2]),
                              np.linspace(-1, 1,
                                          self.velocity_estimate.shape[3]),
                              indexing='ij')
        ...
        for ii in range(N):
            phi = np.power(
                x_slice, px[ii]) * np.power(y_slice, py[ii]) * np.power(z_slice, pz[ii])
