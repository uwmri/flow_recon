...
def svt_inplace(output, input, lamda, blk_shape, blk_strides, block_iter):
    ...
    bmat_shape = (input.shape[0], blk_shape[0] * blk_shape[1] * blk_shape[2])
    for biter in range(block_iter):

        shiftz = np.random.randint(blk_shape[0])
        shifty = np.random.randint(blk_shape[1])
        shiftx = np.random.randint(blk_shape[2])
        ...
        for nz in nb.prange(bz):
            ...
            for ny in range(by):
                ...
                for nx in range(bx):
                    ...
                    # Grab a block
                    for tframe in range(input.shape[0]):
                        count = 0
                        for k in range(sz, ez):
                            for j in range(sy, ey):
                                for i in range(sx, ex):
                                    block[tframe, count] = input[tframe, k % Sz, j % Sy, i % Sx]
                                    count += 1
                    ...
                    for tframe in range(input.shape[0]):
                        ...
                        for k in range(sz, ez):
                            for j in range(sy, ey):
                                for i in range(sx, ex):
                                    output[tframe, k % Sz, j % Sy, i % Sx] += scale*block[tframe, count]
                                    count += 1
...
class SingularValueThresholdingNumba(sp.prox.Prox):
    ...
    def __init__(self, ishape, frames, num_encodes, lamda=None, block_size=8, block_stride=8, axis=0, block_iter=4):
    ...
    def _prox(self, alpha, input):
        ...
        output = svt_inplace(output, input, bthresh, tuple(self.block_shape), tuple(self.block_stride), self.block_iter)
...
class BatchedSenseRecon(sp.app.LinearLeastSquares):
    ...
    def __init__(self, y, mps, lamda=0, weights=None, num_enc=0,
        ...
        # wrap to run GPU
        grad_ops = [sp.linop.ToDevice(op.oshape, self.cpu_device, self.gpu_device) * op * sp.linop.ToDevice(op.ishape,
                                                                                                            self.gpu_device,
                                                                                                            self.cpu_device)
                    for op in grad_ops_nodev]
        ...
        super().__init__(A, y, x=x, proxg=proxg, show_pbar=show_pbar, alpha=1.0, accelerate=True,
                         **kwargs)  # default alpha = 1
    ...
    def _write_log(self):
        ...
        zres = self.x.shape[0] // self.num_images
        out_slice = zres // 2

        self.logger.info(f'Logging slice {out_slice} to file {self.log_out_name}')

        Xiter = xp.copy(self.x[out_slice::zres, ...])
...
def autofov(mri_raw=None, device=None,
            thresh=0.08, scale=1, oversample=2.0):
    ...
    with device:
        ...
        for c in range(mri_raw.Num_Coils):
            ...
            sos += xp.square(xp.abs(sp.nufft_adjoint(ksp_t, coord_t, img_shape)))
