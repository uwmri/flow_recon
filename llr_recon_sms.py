...
class BatchedSenseSMSRecon(sp.app.LinearLeastSquares):
    ...
    def __init__(self, y, mps, sms_factor=1, blips=None, lamda=0, weights=None, num_enc=0, gate_type='time',
        ...
        if fast_maxeig:
            print('Fast Maxeig')
            A = SenseSMS(mps, coord=coord[0], weights=weights[0], sms_factor=sms_factor, blips=blips, ishape=None,
        ...
        #                          show_pbar=self.show_pbar).run()
            ...
            #global max eigen
            ops_list = [SenseSMS(mps, sms_factor=sms_factor, blips=blips, coord=coord[e],  weights=weights[e], ishape=None,
        ...
        if composite_init:
            ...
            for e in range(self.num_images):
                ...
                # Sense operator
                A = SenseSMS(mps, coord=coord[e], weights=weights[e], sms_factor=sms_factor, blips=blips, ishape=None,
                                       coil_batch_size=coil_batch_size, comm=comm)
                data = xp.copy(y[e])
                for s in range(sms_factor):
                    data[...,s] *= weights[e] ** 0.5
            ...
            if weights is not None:
                for e in range(self.num_images):
                    for s in range(sms_factor):
                        y[e][...,s] *= sp.to_device(weights[e] ** 0.5, sp.get_device(y[e]))
        ...
        self.A_list = [SenseSMS(mps, coord=coord[e], weights=weights[e], sms_factor=sms_factor, blips=blips, ishape=None,
                                       coil_batch_size=coil_batch_size, comm=comm) for e in range(self.num_images)]
