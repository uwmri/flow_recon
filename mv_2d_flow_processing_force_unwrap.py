...
if __name__ == '__main__':
    ...
    files_to_compare = find(['CCCardiac*.h5','Time*.h5'], os.getcwd())
    ...
    for idx, f in enumerate(files_to_compare):
        ...
        for i in range(image_sub.shape[0]):
            ...
            if vencs[i] > 8000000:
                ...
                #phase = np.angle(image_sub[i]*np.exp(-1j*math.pi/2.4)) + math.pi/2.4    # this value solves SV75 ICA1 case13
                phase = np.angle(image_sub[i]*np.exp(-1j*math.pi/3.5)) + math.pi/3.5    # this value solves SV75 ICA1 case13


            # add to phase -math.pi/3.5    # this value solves SV75, DV ICA1 case7 (both repeats)
            # add to phase - math.pi/3.5 (this value solves SV75 ICA1 case13)
